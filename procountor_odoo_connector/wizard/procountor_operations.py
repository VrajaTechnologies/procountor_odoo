...
class ProcountorOperations(models.TransientModel):
    ...
    instance_id = fields.Many2one(
        ...
        string='Instance',
        default=_get_default_instance
    )
    procountor_operation = fields.Selection(
        selection=[('import', 'Import')],
        string='Procountor Operations',
        default='import'
    )
    import_operations = fields.Selection(
        selection=[("import_product", "Import Product"),
                   ("import_customers", "Import Customer"),
                   ('import_vats', 'Import Vats')],
        string='Import Operations',
        default='import_product'
    )
    ...
    procountor_product_ids = fields.Char(
        string='Product IDs'
    )
    ...
    def execute_process_of_procountor(self):
        ...
        status, log_name, msg = True, '', ''
        if self.import_operations == "import_product":
            status, log_name, msg = self.env['procountor.product.listing'].procountor_import_product_list(instance,
        ...
        if not status:
            return {
                ...
                'params': {
                    'title': msg,
                    'message': 'Please Check Following Log : {0}'.format(log_name),
                    'type': 'warning',
                    'sticky': False,
